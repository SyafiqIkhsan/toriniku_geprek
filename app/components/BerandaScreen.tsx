  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="bg-primary px-5 md:px-8 pt-8 pb-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-start justify-between">
          <div>
            ...
            <p className="text-on-primary/80 text-sm mt-0.5">Wilujeng Sumping, {userEmail.split("@")[0]}! 👋</p>
          </div>
          <button id="btn-notification" className="w-9 h-9 bg-white/20 rounded-full flex items-center justify-center mt-1 backdrop-blur-sm hover:bg-white/30 active:scale-95 cursor-pointer">
        ...
        <div className="relative z-10 mt-5 bg-white/15 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-on-primary/80 text-xs font-semibold uppercase tracking-wide">Tanggal Hari Ini</p>
          ...
          <p className="text-on-primary/80 text-xs font-semibold uppercase tracking-wide mt-3">Estimasi Laba Bersih</p>
      ...
      <div className="flex-1 px-5 md:px-8 pt-6 pb-24 md:pb-8 bg-background">
        <h2 className="text-base font-bold text-on-surface mb-3">Ringkasan Bisnis</h2>
        ...
        >
          ...
          <div className="flex-1">
            {ordersLoading ? (
              ...
              <>
                <p className="text-xs text-on-surface-variant font-medium">Pesanan Aktif</p>
                <p className="text-lg font-bold text-on-surface">{activeOrders.length} Pesanan</p>
                <p className="text-xs text-outline">{needsProcessing.length} perlu diproses segera</p>
              </>
            )}
          </div>
          <svg className="w-5 h-5 text-outline" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          ...
          >
            <div className="w-10 h-10 bg-tertiary-container/20 rounded-xl flex items-center justify-center flex-shrink-0">
              <svg className="w-5 h-5 text-tertiary" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z" />
                <line x1="3" x2="21" y1="6" y2="6" />
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 11v6m-3-3h6" />
              </svg>
            </div>
            <span className="text-sm font-semibold text-on-surface-variant hover:text-on-surface">+ Pesanan</span>
          </button>

          <button id="btn-tambah-laba" className="bg-surface-container-lowest rounded-2xl p-4 flex items-center gap-3 shadow-ambient hover:shadow-md active:scale-[0.98] text-left cursor-pointer transition-all">
            ...
            <span className="text-sm font-semibold text-on-surface-variant hover:text-on-surface">+ Laba 50K</span>
          </button>
          ...
          <div className="bg-surface-container-lowest rounded-2xl p-4 shadow-ambient">
            ...
            {menuLoading ? <Skeleton className="h-7 w-8 mt-1 mb-1" /> : <p className="text-2xl font-bold text-on-surface mt-1">{activeMenuCount}</p>}
          ...
          <div className="bg-surface-container-lowest rounded-2xl p-4 shadow-ambient">
            ...
            {ordersLoading ? <Skeleton className="h-7 w-8 mt-1 mb-1" /> : <p className="text-2xl font-bold text-primary mt-1">{completedOrders.length}</p>}
