...
export default function BerandaScreen() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="bg-orange-500 px-5 pt-10 pb-6 relative overflow-hidden">
        {/* Decorative circles */}
        <div className="absolute -top-10 -right-10 w-32 h-32 bg-orange-400 rounded-full opacity-50" />
        <div className="absolute top-5 -right-5 w-20 h-20 bg-orange-300 rounded-full opacity-30" />

        <div className="relative z-10 flex items-start justify-between">
          <div>
            <h1 className="text-white text-xl font-bold">Toriniku Geprek</h1>
            <p className="text-orange-100 text-sm mt-0.5">Wilujeng Sumping, Juragan! 👋</p>
          </div>
          <button
            id="btn-notification"
            className="w-9 h-9 bg-white/20 rounded-full flex items-center justify-center mt-1 backdrop-blur-sm hover:bg-white/30 active:scale-95"
          >
        ...
        <div className="relative z-10 mt-5 bg-white/15 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
          ...
          <p className="text-white text-2xl font-bold mt-1">{formatRupiah(totalRevenue)}</p>
        </div>
      </div>

      {/* Body */}
      <div className="flex-1 overflow-y-auto px-5 pt-5 pb-24 bg-gray-50">
        ...
        <button
          ...
          className="w-full bg-white rounded-2xl p-4 flex items-center gap-4 shadow-sm hover:shadow-md active:scale-[0.98] mb-3 text-left"
        >
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <button
            ...
            className="bg-white rounded-2xl p-4 flex items-center gap-3 shadow-sm hover:shadow-md active:scale-[0.98] text-left"
          >
      ...
      <button
        id="fab-pesanan-baru"
        onClick={() => setActiveScreen("pesanan-baru")}
        className="absolute bottom-20 right-5 w-14 h-14 bg-blue-500 rounded-full shadow-lg flex items-center justify-center hover:bg-blue-600 active:scale-95"
      >
