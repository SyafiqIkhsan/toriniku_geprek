...
function OrderCard({ order }: { order: Order }) {
  ...
  return (
    <div id={`order-card-${order.id}`} className="bg-surface-container-lowest rounded-2xl p-4 shadow-ambient hover:shadow-md relative transition-all">
      <div className="flex items-start justify-between mb-2">
        <div>
          <span className="font-bold text-on-surface text-base">#{order.orderNumber}</span>
      ...
      {showMenu && (
        <div className="absolute right-4 top-12 bg-surface-container-lowest rounded-xl shadow-xl border border-outline-variant z-50 py-1 min-w-36 transition-all" onMouseLeave={() => setShowMenu(false)}>
          {statusOptions.map((s) => (
            <button
              key={s}
      ...
      {order.notes && <p className="text-xs text-outline italic mb-2">{order.notes}</p>}
  ...
  const getGroupedOrders = (ordersList: Order[]): GroupedOrders[] => {
    ...
    if (groupBy === "type") {
      ...
      return [
        {
          title: "Dine In",
          badgeColor: "bg-secondary-container text-on-secondary-container border border-outline-variant",
          icon: (
        ...
        {
          title: "Takeaway",
          badgeColor: "bg-primary-container text-on-primary-container border border-primary/20",
          icon: (
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
    ...
    if (groupBy === "status") {
      const statuses: OrderStatus[] = ["Baru", "Diproses", "Selesai", "Dibatalkan"];
    ...
    if (groupBy === "date") {
      ...
      return [
        ...
        {
          title: "Sebelumnya",
          badgeColor: "bg-surface-container text-on-surface-variant border border-outline-variant",
          icon: (
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="bg-primary px-5 md:px-8 pt-8 pb-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            ...
            <p className="text-on-primary/80 text-sm mt-0.5">Pantau semua orderan masuk di sini</p>
          </div>
          <button
      ...
      <div className="bg-surface-container-lowest px-4 md:px-8 py-2 flex items-center gap-3 border-b border-outline-variant flex-wrap flex-shrink-0 transition-all">
        <span className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Kelompokkan:</span>
      ...
      <div className="flex-1 px-4 md:px-8 py-4 pb-24 md:pb-8 bg-background">
        {ordersLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-surface-container-lowest rounded-2xl p-4 shadow-ambient space-y-2 animate-pulse">
              ...
              {getGroupedOrders(paginatedOrders).map((group) => (
                <div key={group.title} className="space-y-3">
                  ...
                  <div className="flex items-center gap-2 pb-1 border-b border-outline-variant/60">
                    <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${group.badgeColor}`}>
                      {group.icon}
                      <span>{group.title}</span>
                    </div>
                    <span className="text-xs font-bold text-outline">({group.orders.length} Pesanan)</span>
